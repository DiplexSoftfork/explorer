  ...
  function express_app() {
    ...
    app.get('*', hasAccess, function(req, res) {
      ...
      client.cmd(command, function(err, response) {
        if (err) {
          console.log(err);
          res.send("There was an error. Check your console." + err);
        } else {
    ...
    function specialApiCase(method_name, query_parameters) {
      ...
      switch (method_name) {
        ...
        case 'getrawtransaction':
          ...
          break;
/*        case 'createrawtransaction':  //Bro, it's for you.
          for (var parameter in query_parameters) {
            if (query_parameters.hasOwnProperty(parameter)){
              if (parameter == 'params')  //'transactions'
                params.push(query_parameters[parameter]);
              //else if (parameter == 'addresses')
              //  params.push(query_parameters[parameter]);
              }
          }

          var cmd = prepareRpcCommand(settings.api_cmds.createrawtransaction, params);
        ...
        case 'sendrawtransaction':
          for (var parameter in query_parameters) {
            if (query_parameters.hasOwnProperty(parameter) && parameter == 'signedhex')
              params.push(query_parameters[parameter]);
          }
